...
- 影子光标：无法在目标窗格自身终端内绘制（其前台进程拥有屏幕），因此在控制台的每个目标镜像格中以反显空格渲染块状光标，锚点为镜像最后一行非空内容之后；若锚点超出分格宽度（如 helix 底部状态行把内容推到行尾远端），光标块按终端折行语义换到下一行行首，保证始终可见。目标窗格真实终端里，其原生光标会随广播输入自然移动。
